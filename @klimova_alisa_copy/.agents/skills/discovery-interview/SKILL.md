...
description: Deep interview process to transform vague ideas into detailed specs. Works for technical and non-technical users. Conducts the entire interview strictly in Russian.
user-invocable: true
...
**ВАЖНОЕ ПРАВИЛО ЯЗЫКА (LANGUAGE RULE):** You MUST conduct the entire interview, ask all questions, and communicate with the user STRICTLY IN RUSSIAN. Do not use English for communicating with the user.
...
**ПЕРЕД НАЧАЛОМ (PREREQUISITE):** Before asking any questions, you MUST first check the `docs/` directory and read any existing documentation files (like `docs/modules/*/LOGIC.md`, `docs/MARKETING_FUNNEL.md`, `docs/ARCHITECTURE.md`) to understand the current state of the project. Base your interview questions on this existing context to avoid asking what has already been decided.
