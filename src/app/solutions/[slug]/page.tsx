...
export default async function NichePage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="bg-white min-h-screen">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-12 max-w-3xl">
            {niche.features.map((feature, i) => (
              <div key={i} className="flex items-center gap-3">
                ...
                <span className="font-sans text-ink font-medium">{feature}</span>
              </div>
            ))}
          </div>
      ...
      <ServicesSlider title={niche.name.toUpperCase()} />
          ...
          <div className="relative bg-ink rounded-[2rem] p-8 lg:p-16 flex flex-col lg:flex-row gap-12 overflow-hidden shadow-2xl">
            ...
            <div className="relative z-10 lg:w-2/3 flex flex-col justify-center">
              <div className="space-y-6 font-sans text-white/70 text-base md:text-lg leading-relaxed font-light">
                <p>
                  Я глубоко погружаюсь в специфику каждого проекта. Если вам нужно {niche.name.toLowerCase()}, я не просто нарисую дизайн, а продумаю логику, которая будет удобна вашим клиентам и принесет реальную прибыль.
                </p>
                <p>
