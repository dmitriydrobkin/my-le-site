...
export default function AlisaKlimovaProposalFull() {
  return (
    <div className="min-h-screen bg-surface selection:bg-coral/20 font-sans text-ink overflow-x-hidden">
      {/* Background Blobs */}
      ...
      <main className="relative z-10 container mx-auto px-5 py-12 md:py-20 max-w-4xl space-y-24">
        ...
        <motion.section initial="hidden" animate="visible" variants={STAGGER} className="text-center pt-8">
          ...
          <motion.h1 variants={FADE_UP} className="font-display text-4xl md:text-6xl font-black mb-6 leading-tight">
            Разработка персонального <br className="hidden md:block" />
            <span className="text-coral">сайта эксперта</span>
          </motion.h1>
          <motion.div variants={FADE_UP} className="flex flex-wrap justify-center gap-4 text-sm font-medium text-ink/60 mb-8">
            <span className="px-4 py-2 bg-white rounded-xl shadow-sm border border-black/5">👤 Клиент: <strong>Алиса Климова</strong></span>
            <span className="px-4 py-2 bg-white rounded-xl shadow-sm border border-black/5">💻 Платформа: <strong>Tilda Publishing</strong></span>
          </motion.div>
          <motion.p variants={FADE_UP} className="text-lg text-ink/70 max-w-2xl mx-auto leading-relaxed">
        ...
        <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={STAGGER} className="space-y-8">
          <motion.div variants={FADE_UP}>
             <h2 className="text-3xl md:text-4xl font-black font-display flex items-center gap-4">
               <span className="w-10 h-10 rounded-xl bg-coral text-white flex items-center justify-center text-xl shadow-neon-coral">1</span>
               Объем работ (Что входит в стоимость)
             </h2>
          </motion.div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <motion.div variants={FADE_UP} className="p-8 bg-white/70 backdrop-blur-xl shadow-glass rounded-[2rem] border border-white/50">
              <h3 className="text-xl font-bold font-display mb-6 flex items-center gap-3">
                <FileText className="text-coral w-6 h-6" /> Структура разделов
              </h3>
              <ul className="space-y-4">
                {[
                  ...
                  { title: "Статьи", desc: "Блог: настройка функционала + загрузка первых 3-5 статей для примера." },
            ...
            <motion.div variants={FADE_UP} className="p-8 bg-white/70 backdrop-blur-xl shadow-glass rounded-[2rem] border border-white/50">
              <h3 className="text-xl font-bold font-display mb-6 flex items-center gap-3">
                <Settings2 className="text-coral w-6 h-6" /> Техническая настройка
              </h3>
              <ul className="space-y-4">
                {[
        ...
        <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={STAGGER} className="space-y-8">
          <motion.div variants={FADE_UP}>
             <h2 className="text-3xl md:text-4xl font-black font-display flex items-center gap-4">
               <span className="w-10 h-10 rounded-xl bg-coral text-white flex items-center justify-center text-xl shadow-neon-coral">2</span>
               Этапы работы (Пошаговое согласование)
             </h2>
             <p className="mt-4 text-ink/70 text-lg">Мы работаем по жесткой системе, чтобы вы не получили «кота в мешке» и контролировали каждый шаг.</p>
          </motion.div>

          <div className="space-y-4 relative before:absolute before:inset-y-4 before:left-8 before:w-0.5 before:bg-coral/20">
            {[
              ...
              <motion.div variants={FADE_UP} key={index} className="relative pl-24 pr-8 py-8 bg-white shadow-glass hover:shadow-glass-hover transition-shadow rounded-[2rem] border border-black/5 group">
                <div className="absolute left-4 top-1/2 -translate-y-1/2 w-9 h-9 bg-coral text-white font-bold font-display rounded-full flex items-center justify-center shadow-[0_0_0_6px_rgba(255,77,77,0.1)] group-hover:scale-110 transition-transform">
                  {item.step}
                </div>
                <div className="flex justify-between items-start mb-2 gap-4">
                  <h4 className="text-xl font-bold font-display text-ink">{item.title}</h4>
                  <span className="px-3 py-1 bg-coral/10 text-coral font-bold rounded-lg text-sm whitespace-nowrap">{item.price}</span>
                </div>
                <p className="text-ink/70 leading-relaxed text-sm md:text-base">{item.desc}</p>
              </motion.div>
            ))}
          </div>
        </motion.section>
        ...
        {/* 3. Бюджет и сроки */}
        <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={STAGGER} className="space-y-8">
          <motion.div variants={FADE_UP} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            
            <div className="p-8 md:p-10 bg-ink text-white rounded-[2rem] shadow-2xl relative overflow-hidden">
              <div className="absolute top-0 right-0 w-48 h-48 bg-coral/20 rounded-full blur-[60px]"></div>
              <h3 className="text-2xl font-bold font-display mb-8 flex items-center gap-3 relative z-10">
                <CreditCard className="w-6 h-6 text-coral" /> Бюджет и расчеты
              </h3>
              <div className="space-y-6 relative z-10">
                <div className="flex justify-between items-end border-b border-white/10 pb-4">
                  <span className="text-white/60">Итоговая стоимость:</span>
                  <span className="text-4xl font-black font-display">$350</span>
                </div>
                ...
                <div className="p-4 bg-white/5 rounded-xl border border-white/10 text-xs text-white/60 leading-relaxed mt-4">
                  <strong>Способ:</strong> Оплата рублями с личной карты РФ через безопасную P2P-сделку (перевод по актуальному курсу доллара на день оплаты).
                </div>
              </div>
            </div>

            <div className="p-8 md:p-10 bg-white shadow-glass rounded-[2rem] border border-black/5 flex flex-col justify-between">
              <div>
                <h3 className="text-2xl font-bold font-display mb-8 flex items-center gap-3 text-ink">
                  <ShieldCheck className="w-6 h-6 text-coral" /> Поддержка и гарантии
                </h3>
                <ul className="space-y-5">
                  <li className="flex gap-4">
                    <CheckCircle2 className="w-6 h-6 text-coral shrink-0" />
                    <div>
                      <strong className="block text-ink mb-1">Полные права на сайт</strong>
                      <span className="text-ink/70 text-sm">Домен и хостинг оформляются на вас. Все доступы передаются вам, никакой привязки к аккаунту разработчика.</span>
                    </div>
                  </li>
                  <li className="flex gap-4">
                    <Smartphone className="w-6 h-6 text-coral shrink-0" />
                    <div>
                      <strong className="block text-ink mb-1">Обучение</strong>
                      <span className="text-ink/70 text-sm">Запись подробной видеоинструкции, как вам самостоятельно менять цены, тексты и добавлять новые статьи без помощи программиста.</span>
                    </div>
                  </li>
                </ul>
              </div>
              <div className="mt-8 p-4 bg-green-50 rounded-xl border border-green-100 text-sm text-green-800 font-medium">
        ...
        <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={STAGGER} className="space-y-6">
           <motion.div variants={FADE_UP} className="p-8 md:p-10 bg-coral/5 rounded-[2.5rem] border border-coral/10">
             <div className="flex items-center gap-4 mb-6">
               <div className="w-12 h-12 bg-white rounded-xl shadow-sm flex items-center justify-center">
                 <Database className="w-6 h-6 text-coral" />
               </div>
               <h3 className="text-2xl font-bold font-display">Перспективы масштабирования</h3>
             </div>
        ...
        {/* CTA */}
        <motion.section initial="hidden" whileInView="visible" viewport={{ once: true }} variants={FADE_UP} className="pb-24 text-center">
           <h3 className="text-2xl font-bold font-display mb-8 text-ink">Если всё корректно — жму руку и можем начинать работу! 😉</h3>
           <a href="https://t.me/dima_malyshev" target="_blank" rel="noreferrer" className="inline-block px-12 py-5 bg-coral text-white font-bold rounded-2xl shadow-neon-coral text-lg hover:scale-105 transition-transform">
             Утвердить и начать работу
           </a>
        </motion.section>
